    ...
    for(step=1; step<steps; step++){
        ...
        while(i < n_population){
            ...
            while(equals == 1){
                ...
                if(equals == 0) equals = check_equals(&(child_population.population[i]), &(child_population.population[i+1]), n);*/
                LOP_objective_function(&(child_population.population[i]), n);
                (*evaluations) += 1;
                if(child_population.population[i].obj_func_value < (min-int(min/10)) && equals == 0){
                    equals = 1;
                }
                LOP_objective_function(&(child_population.population[i+1]), n);
                (*evaluations) += 1;
                if(child_population.population[i+1].obj_func_value < (min-int(min/10)) && equals == 0){
                    equals = 1;
                }
            }
            i+=2;
        ...
        if(t_criterion >= criterion_value) 
            step = steps; // finish
